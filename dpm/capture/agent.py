...
async def visual_explore(url: str, model, output_root: Path):
    """Navigates the site using vision, types fake data, and captures the state.
    
    Args:
        url: Target website URL
        model: dpm.ai.client.Model instance for AI-driven navigation
        output_root: Root output directory (e.g., Path('out'))
        
    Returns:
        Tuple of (steps_log, reject_click_depth, final_step_name, is_blocked, all_signals, all_errors, meta)
    """
    ...
    async with async_playwright() as p:
        ...
        for step in range(max_steps):
            ...
            lean_history.append(types.Content(role="model", parts=[types.Part(text=str(response))]))
            ...
            # Resolution Logic: fill placeholders from element_map
            unresolved = []
